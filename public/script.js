...
function createViewHandler(path) {
    if (path.toLowerCase() === '/registration')
        return new RegistrationViewHandler()
...
class LoginViewHandler extends ViewHandler {
    validation() {
        return validation(false)    //bad side-effect
    }
}

class RegistrationViewHandler extends ViewHandler {
    months = [
...
const viewHandler = createViewHandler(window.location.pathname)
viewHandler.service()
